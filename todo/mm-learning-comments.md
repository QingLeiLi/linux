...
- [x] `mm/sparse-vmemmap.c`：616/142/0.231/10；仅新增 142 行中文学习注释。已按第 17 章复核普通/启动/altmap backing 分配、PGD 至 PTE 的建表发布、huge PMD 回退、HVO tail 镜像与写保护、ZONE_DEVICE compound 页复用及页引用、subsection 位图、热插拔 section 激活/撤销、usage 的 RCU 释放和 memmap 计数配对；密度门禁、diff check 与补丁模式 checkpatch 通过。关联读取为 `mm/{sparse,memory_hotplug,memremap}.c` 与 `include/linux/mmzone.h` 的 section/altmap/hotplug 契约；本轮未修改关联文件，学习注释覆盖仍按各自基线处理。
- [x] `mm/mmu_notifier.c`：648/154/0.238/10；仅新增 156 行中文学习注释与空行。已按第 17 章复核 subscriptions 的 SRCU/list/itree 生命周期、interval 奇偶序列碰撞重试、start/finish 与 deferred 更新、release 的全 VA 清理、young 位/secondary TLB 广播、non-blocking `-EAGAIN` 补偿、注册发布、mm_count 与 users 配对、异步 `put` 回收、interval insert/remove 等待边界；密度门禁、diff check 与补丁模式 checkpatch 通过。关联读取为 `include/linux/mmu_notifier.h` 及 `mm/memory.c` 的 range 调用约定；本轮未修改关联文件，学习注释覆盖仍按各自基线处理。
